...
def get_authorization_url(
    ...
    **Documentation**: https://forge.autodesk.com/en/docs/oauth/v2/reference/http/authorize-GET
    ...
    url = url + f"&response_type={(response_type)}"
    url = url + f"&redirect_uri={(quote(redirect_uri))}"
    url = url + f"&scope={quote(' '.join(map(lambda s: s.value, scopes)))}"
    if state:
...
class AuthenticationClient(BaseClient):
    ...
    **Documentation**: https://forge.autodesk.com/en/docs/oauth/v2/reference/http
    ...
    def authenticate(
        self, client_id: str, client_secret: str, scopes: list[Scope]
    ) -> dict:
        """
    ...
    def get_token(
        ...
        **Documentation**: https://forge.autodesk.com/en/docs/oauth/v2/reference/http/gettoken-POST
        ...
        headers = {"Authorization": f"Basic {code_challenge}"}
        return self._post("/token", form=form, headers=headers).json()
